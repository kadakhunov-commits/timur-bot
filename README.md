...
`OBSHAK_PATH` опционален и переопределяет путь к JSON-хранилищу общака (по умолчанию `data/obshak.json`; на Amvera имеет смысл поставить `/data/obshak.json`, чтобы данные пережили передеплой).
