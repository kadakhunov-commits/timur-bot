...
`/miniapp` — приложение **«Соседи»**: пиксельная кухня, где четверо соседей (Рустем, Кадыр,
...
- **Рулетка «кто идёт в магазин»** — пиксельный барабан с аватарами. Рулетка подравнивает:
  кто ходил чаще, у того шанс ниже (окно — 14 дней). У броска есть кулдаун и суточный лимит,
  победитель не выпадает дважды подряд, а кнопка «я сходил» отмечает настоящий поход в магазин.
...
- **Выключатель и лампочка** — тап по выключателю гасит свет на кухне, и это общее состояние:
  соседи увидят тёмную кухню, кот комментирует.
- **Погода в окне** — дождь, снег, туман, гроза или просто облака по реальной погоде города
  из `config/obshak.yaml`; ночью в окне луна, днём солнце. Температура — в строке под сценой.
- **Кот-подъёбус** — при входе Барсик говорит в облачке колкость по ситуации: сколько дней
  ...
  Та же фраза попадает в карточку `/obshak`, если человек привязан к аватару. Кота можно
  гладить (с дневным лимитом), а его уровень растёт от опыта: покупки, серия дней и квесты.
- **Цифры** — бюджет месяца с прогрессом и алертом, средний чек, крупнейшая покупка, пирог
  по категориям и прогноз трат до конца месяца по текущему темпу.
- **Записки на холодильнике** — короткие сообщения соседям прямо на дверце.
- **Корона месяца** — победитель месяца носит золотую корону на кухне, история корон хранится.
- **Секретные кубки** — ачивки, условие которых скрыто, пока они не откроются, плюс прогресс
  до следующего кубка в зале славы.

Уведомления в беседу. Если задан `notify_chat_id` (или `chat_id`, или карточка уже закреплена),
бот сам пишет в беседу: записанная покупка, новый запрос «скиньтесь», оплата, закрытый квест
недели и алерт по бюджету. По понедельникам туда же уходит «газета недели» с итогами.
Кнопка «пнуть» у запроса реально напоминает должникам упоминанием (с кулдауном 6 часов).

Быстрая запись из чата. `/obshak 500 майонез` (или `/obshak 500 майонез еда`) пишет покупку,
не открывая миниапп: категория подставляется из прошлых покупок с тем же названием.
...
- `/obshak 500 майонез [категория]` — записать покупку прямо в чат, без миниаппа;
...
В интерфейсе приложение называется «Соседи»: так подписаны шапка, новости, меню бота и грамоты.
Во внутренних именах (модуль `obshak`, путь `/api/obshak`, конфиг `config/obshak.yaml`) остаётся
прежнее имя — это техническая сторона, пользователь её не видит.

Шрифты (`Press Start 2P` и `Pixelify Sans`) лежат в `miniapp/public/obshak/fonts/` и раздаются с
нашего домена. На Google Fonts кириллица — отдельный файл, который грузится лениво и иногда не
доезжает; тогда кириллический текст оставался системным шрифтом и выглядел кривым.
...
- `config/obshak.yaml` — состав общака, категории расходов, пороги уровней банки и кубков, фразы кота и его подколы, квесты недели, шаблоны новостей, город для погоды, бюджет месяца (`monthly_budget`, пороги алертов), правила рулетки (`roulette`), секретные ачивки (`secret_achievements`) и уведомления (`notifications`, `notify_chat_id`).
