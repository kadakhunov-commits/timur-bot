...
Кнопка в беседе. Telegram не разрешает `web_app`-кнопки в группах, поэтому бот постит
deep-link на Main Mini App:
...
- при добавлении бота в беседу карточка появляется сама;
- `pin_group_card: true` в `config/obshak.yaml` закрепит её (боту нужны права на закреп);
