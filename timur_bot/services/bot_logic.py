...
async def call_openai_vision(
    ...
    mood_line = (
        ...
        "пусть это мягко влияет на тон."
    )
    direct_reply_max_chars = int(_adaptive_humor_settings(memory)["direct_reply_max_chars"])
    text_context = (
        "тебе прислали картинку в чате — часто скриншот новости, мем или фото. "
        "отреагируй коротко как живой собеседник: можно просто по-человечески прокомментировать или сухо подколоть. шути необязательно, шутка по приказу будет кринжем. "
        "категорически запрещено придумывать: выдуманные названия, прозвища, факты и то, чего на картинке нет. если это новость — отреагируй по сути самой новости и не перевирай её. "
        f"одна короткая фраза без подводок, максимум {direct_reply_max_chars} знаков, маленькими буквами, без эмодзи. "
        + mood_line
    )
