...
GLOBAL_DAILY_VISION_LIMIT = 50
CHAT_DAILY_VISION_LIMIT = 5
USER_DAILY_VISION_LIMIT = 5

# Вероятности поведения
BASE_REPLY_CHANCE = 0.08
CHAIN_REPLY_CHANCE = 0.16
MEM_REPLY_CHANCE = 0.08
PHOTO_RANDOM_REPLY_CHANCE = 0.22

MEMES = [
    "https://i.imgur.com/your_meme1.jpg",
    "https://i.imgur.com/your_meme2.jpg",
]
...
RUS_STOPWORDS = {
    "это", "как", "что", "когда", "потом", "только", "чтобы", "просто", "меня", "тебя", "него", "нее",
    "них", "здесь", "там", "тут", "тогда", "сейчас", "очень", "вообще", "ладно", "короче", "типа",
    "блин", "чел", "щас", "будет", "было", "были", "есть", "еще", "ещё", "или", "если", "потому",
    "давай", "даже", "тоже", "твой", "твоя", "твои", "моей", "мой", "моя", "мои", "нам", "вам",
    "мне", "тебе", "его", "ее", "её", "кто", "куда", "зачем", "почему", "который", "которые",
}

EN_STOPWORDS = {
    "this", "that", "with", "have", "just", "from", "your", "you", "they", "them", "about", "what",
    "when", "where", "will", "would", "could", "should", "there", "here", "then", "than", "very",
}

PROFANITY_MARKERS = {
    "бля", "блять", "нах", "нахуй", "хер", "хуй", "сука", "пизд", "еб", "жоп", "мраз", "уеб", "еба",
}

ARCHETYPE_LEXICON = {
    "драматик": {"скандал", "ссора", "разнос", "драма", "обида", "предал", "развод", "ревность"},
    "мемолог": {"мем", "ор", "ржака", "угар", "лол", "кринж", "кек", "прикол"},
    "спортсмен": {"зал", "треня", "кач", "пресс", "кардио", "бег", "становая", "протеин"},
    "айтишник": {"код", "баг", "сервер", "деплой", "api", "питон", "гит", "фича"},
...
ты — тимур, 22-летний пацан из казани. ты живешь в казани, учился в школе №18, 1–9 класс в б классе (углубленный английский),
потом 10–11 а класс, классная — ирина владимировна. после школы поступил в кфу, сначала в итис, потом ушел на мехмат кфу.
ты общаешься как живой дерзкий чел: токсично, едко и смешно, но без реальной травли, угроз и жести.
твой стиль — прожарка по-дружески: подколы, сарказм, черный юмор, локальные мемы чата.
если видишь, что человеку плохо или тема серьезная, резко снижаешь токсичность и отвечаешь по-человечески.
иногда вспоминаешь как митя понесло и он удалил все сообщения кадыра — как локальный мем.
говоришь всегда с маленькой буквы, без эмодзи, почти без пунктуации, сообщения короткие.
максимум 2 предложения, очень коротких. иногда можно 3 супер коротких.
иногда можешь ответить цепочкой из 2–3 сообщений подряд, как будто дописал мысль.
ты не говоришь, что ты бот или нейросеть. не произносишь слова "нейросеть", "chatgpt", "openai", "модель".
не объясняешь как ты думаешь, просто отвечаешь.
""".strip()
...
def detect_archetype_scores(text: str, keywords: List[str]) -> Dict[str, int]:
    ...
    payload = set(extract_keywords(text, limit=25) + keywords)
